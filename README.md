...
1. **접속 정보 설정.** 저장소 루트의 `.env.example`을 `.env`로 복사하고 실제 값을 채운다. `.env`는 `.gitignore`로 제외되어 있어 커밋되지 않는다.
   ...
   노트북의 DB 연결 셀이 `python-dotenv`로 이 파일을 자동으로 읽는다(`pip install -r requirements.txt`에 포함). `.env` 대신 셸 환경변수로 `RECRUIT_DB_URL`을 직접 내보내도 동작한다.
2. `.venv`를 활성화한 뒤 `notebooks/` 폴더를 기준 디렉터리로 열어서 실행한다(모든 상대경로가 `notebooks/` 기준이고, 캐시는 `_cache/` 밑에 있다). Jupyter 커널은 `.venv` 인터프리터를 선택.
3. 01, 02, 04는 각자 `.pkl` 캐시가 있어 `REFRESH = False`인 이상 로컬 MySQL 없이도 캐시로 재현 가능하다.
4. **05는 캐시가 있어도 셀 초반 몇 개(전처리 테이블 존재 확인 등)는 무조건 DB에 접속**하므로 로컬 MySQL이 켜져 있어야 한다. 접속 정보는 1번에서 설정한 `.env`를 따른다.
5. 03의 리텐션 원본 쿼리는 캐시가 없어 매번 DB에서 새로 조회한다 — 데이터가 크면 시간이 걸릴 수 있다.
6. 노트북에 저장된 출력은 **차트와 `print`로 찍은 요약 수치**뿐이다. 데이터프레임 표는 저장하지 않으므로, 표 형태로 보려면 각자 실행해야 한다.

## 데이터 취급

이 저장소에는 제공받은 원본 데이터가 포함되어 있지 않다. 계약상 원본 데이터의 복제·배포·공개가 금지되어 있어 다음 원칙을 따른다.

- 원본 DB 덤프·캐시(`.sql` / `.pkl` / `.parquet`)와 행 단위 데이터를 담은 CSV는 모두 `.gitignore`로 제외한다.
...
- 5개 노트북은 통일된 목차 형식(`## N.` / `### N-M.` / `#### 해석`)을 따른다.
